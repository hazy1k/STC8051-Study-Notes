...
;=================== 检测200ms是否到 ==================================
	...
	CLR		C
	MOV		A, msecond_L	;msecond - 200
	SUBB	A, #LOW 200
	MOV		A, msecond_H
	SUBB	A, #HIGH 200
	JC		L_Main_Loop		;if(msecond < 200), jmp
...
L_BandgapLoop:
	MOV		A, #ADC_CH0
	LCALL	F_Get_ADC10bitResult	;读内部基准ADC, P1ASF=0, 读0通道, 查询方式做一次ADC, 返回值(R6 R7)就是ADC结果, == 1024 为错误
	MOV		A, BandgapL
	ADD		A, R7
	MOV		BandgapL, A
	MOV		A, BandgapH
	ADDC	A, R6
	MOV		BandgapH, A
	DJNZ	R2, L_BandgapLoop
	
	MOV		R4, #0		; ADC = 1024 * Uref / Ux, 则 Ux = 1024 * Uref / ADC = 1024 * 1.25/ ADC = 1280 / ADC
	MOV		R5, #0x1F	; 1024 * 1.25 * 100 * 16 = 2048000 = 0x1F4000, 计算时放大了100倍, 电压有两位小数, 16次ADC值.
	MOV		R6, #0x40
	MOV		R7, #0x00
	MOV		R2, BandgapH
	MOV		R3, BandgapL
...
F_ADC_config:
	MOV		P1ASF, #0			; 读内部基准ADC, P1ASF=0, 读0通道

...
;================= 300ms到 ====================================
	...
	MOV		P1ASF, #0	;
	MOV		A, #ADC_CH0
	LCALL	F_Get_ADC10bitResult	;ACC - 通道0~7, 改变P1ASF后先读一次并丢弃结果, 让内部的采样电容的电压等于输入值.
	MOV		A, #ADC_CH0
	LCALL	F_Get_ADC10bitResult	;读内部基准ADC, P1ASF=0, 读0通道, 查询方式做一次ADC, 返回值(R6 R7)就是ADC结果, == 1024 为错误
	MOV		BandgapH, R6			;保存Bandgap
	MOV		BandgapL, R7
	
	MOV		P1ASF, #ADC_P13			; 设置要做ADC的IO,	ADC_P10 ~ ADC_P17(或操作),ADC_P1_All
	MOV		A, #ADC_CH3
	...
	MOV		R5, ADC3_L
	MOV		R7, #123		; Bandgap为1.23V, 定点计算, 放大100倍
...
F_ADC_config:
	...
	ORL		P1M1, #ADC_P13			; 把ADC口设置为高阻输入
	ANL		P1M0, #NOT ADC_P13
;	SETB	EADC					;中断允许
;	SETB	PADC					;优先级设置
	RET
